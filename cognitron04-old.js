  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        method: 'GET',
        headers: {
          'User-Agent': 'Cognitron AI Assistant/1.0.4'
        }
      };
        ...
        res.on('end', () => {
          try {
            ...
            if (searchResults.length === 0) {
              searchResults.push({
                title: 'Search Query',
                snippet: `Searched for: "${query}". Try refining your search terms or being more specific.`,
                url: '',
                source: 'DuckDuckGo',
                type: 'no_results'
              });
...
function createSystemMessage(reasoningLevel = 'low', agenticMode = false) {
  const baseMessage = "You are Cognitron, a helpful AI assistant with memory and web search capabilities. You can search the web for current information, news, and facts. Be concise, accurate, and friendly.";
  ...
  try {
    ...
    if (hasToolCalls && toolCalls.length > 0) {
      ...
      for (const toolCall of toolCalls.slice(0, CONFIG.MAX_TOOL_CALLS_PER_RESPONSE)) {
        try {
          ...
          if (toolCall.function.name === 'search_conversation_history') {
            ...
            // Handle web search specially (async)
            const args = toolCall.function.arguments;
            const webResults = await searchWeb(args.query, args.max_results || 5);
            
            const result = {
              success: webResults && webResults.length > 0 && webResults[0].type !== 'error',
              message: webResults && webResults.length > 0 ? 
                `Found ${webResults.length} web result${webResults.length === 1 ? '' : 's'}` :
            ...
            if (CONFIG.SHOW_AGENT_DECISIONS) {
              ...
              console.log(chalk.blue(`   ${icon} search_web(${JSON.stringify(args, null, 0)})`));
              console.log(chalk.blue(`      → ${result.message}`));
              if (result.details) {
                console.log(chalk.gray(`      📝 ${result.details}`));
              }
              ...
              if (webResults && webResults.length > 0) {
                console.log(chalk.gray('      🌐 Web Results:'));
                webResults.forEach((item, idx) => {
                  const preview = item.snippet.substring(0, 80);
                  console.log(chalk.gray(`        ${idx + 1}. [${item.source}] ${item.title}`));
                  console.log(chalk.gray(`           ${preview}${item.snippet.length > 80 ? '...' : ''}`));
                  if (item.url) {
            ...
            if (result.success && toolCall.function.name !== 'search_conversation_history' && toolCall.function.name !== 'search_web') {
      ...
      if (settingsChanged) {
        ...
        // No settings changed, but tools were called
        // Check if web search was performed and needs follow-up
        const hasWebSearch = toolResults.some(result => result.webResults);
        ...
        if (hasWebSearch) {
          ...
          // Make a follow-up request with web search results in context
          const followUpOptions = { ...options, agenticMode: false };
          const webSearchResults = toolResults.filter(result => result.webResults);
          
          // Check if we have meaningful web search results
          if (webSearchResults.length > 0) {
            const webSearchSummary = webSearchResults
              .map(result => result.webResults
                .map(item => `${item.title}: ${item.snippet}`)
                .join('\n'))
              .join('\n\n');
            
            const contextMessage = `${message}\n\n[Web search results for reference:\n${webSearchSummary}]\n\nBased on the search results above, please provide a helpful response.`;
            
            return await chatWithGroq(contextMessage, conversationHistory, followUpOptions);
          } else {
            // No meaningful web search results, provide a direct response
            if (CONFIG.SHOW_AGENT_DECISIONS) {
              console.log(chalk.gray('No meaningful web search results found, providing direct response.\n'));
            }
            const noResultsMessage = "I searched the web but couldn't find current information about that topic. Let me provide what I can from my knowledge base.";
    ...
    // Log assistant response to memory system (if response exists)
    if (response) {
    ...
    rl.question(chalk.green('You: '), async (input) => {
      ...
      try {
        ...
        if (response) {
          ...
          // If no response, still add user message to history but show error
          conversationHistory.push({ role: "user", content: trimmedInput });
          console.log(chalk.red('No response generated. This might be due to a processing error.'));
        }
      } catch (error) {
